import { classifyGridFlow, escapeHtml, formatWatts, peakRisk, readPowerWatts, recordRollingSample, rollingWindowStats, predictDemandRisk, formatPredictionSummary, toNumber } from './power_math.js';
...
function peakBadge(stats, threshold) {
  if (!threshold || threshold <= 0) return '';
  const pct = ((stats.current || 0) / threshold * 100).toFixed(0);
  const cls = pct >= 100 ? 'yes' : 'no';
  return `<span class="peak-badge ${cls}">${pct}%</span>`;
}
...
function renderEditor(config, hass) {
  ...
  const sensorOptions = hass && hass.states ? Object.keys(hass.states)
    .filter(eid => {
      const state = hass.states[eid];
      const unit = String(state?.attributes?.unit_of_measurement || '').toLowerCase();
      return unit === 'w' || unit === 'kw' || unit === 'mw';
    })
    .map(eid => `<option value="${escapeHtml(eid)}"${entities && Object.values(entities).includes(eid) ? ' selected' : ''}>${escapeHtml(eid)}</option>`)
    .join('') : '<option value="">No power sensors found</option>';
...
class LivePowerDashboardCard extends HTMLElement {
  ...
  set hass(hass) {
    this._hass = hass;
    if (this._inEditor) { this.render(); return; }
  ...
  async _loadPreset(presetId) {
    ...
    this.config.global_max_w = preset.global_max_w || undefined;
    this.render();
  }
  ...
  async _saveAsPreset() {
    ...
    if (!id) return;
    if (!/^[a-zA-Z0-9_-]+$/.test(id)) { alert('Invalid ID'); return; }
    const payload = {
      id,
    ...
    try {
      ...
      if (!resp.ok) { alert('Failed to save preset'); return; }
  ...
  render() {
    ...
    // Annotate circuits with their current wattage for sorting and bar rendering
    const annotated = circuits.map(c => ({
      ...c,
      _watts: readPowerWatts(this._hass, c.entity) ?? 0,
    }));
    // Sort by power descending so highest consumers appear at the top
    annotated.sort((a, b) => b._watts - a._watts);
    const circuitRows = annotated.map(c => {
      const w = c._watts;
      const max = globalMax > 0
        ? globalMax
        : toNumber(c.max_power || cfg.default_circuit_max_w || load || 1, 1);
  ...
  _attachEventListeners() {
    ...
    byId('edit-btn')?.addEventListener('click', () => { this._inEditor = !this._inEditor; this.render(); });
    byId('editor-cancel')?.addEventListener('click', () => { this._inEditor = false; this.render(); });
    byId('editor-save')?.addEventListener('click', () => this._saveEditor());
    byId('editor-add-circuit')?.addEventListener('click', () => {
      (this.config.circuits ||= []).push({ name: '', entity: '', max_power: '' });
      this.render();
    });
    root.querySelectorAll('[data-remove-circuit]').forEach(btn => {
      btn.addEventListener('click', () => {
        const idx = parseInt(btn.getAttribute('data-remove-circuit'));
        if (!isNaN(idx)) { this.config.circuits?.splice(idx, 1); this.render(); }
      });
    });
    ...
    byId('preset-refresh')?.addEventListener('click', async () => { await this._fetchPresets(); this.render(); });
  }
  ...
  _saveEditor() {
    ...
    const entities = {};
    ['grid','solar','battery','load'].forEach(k => { const v = val('editor-' + k); if (v) entities[k + '_power'] = v; });
    ...
    root.querySelectorAll('[data-circuit-field="name"]').forEach((f, i) => {
      ...
      if (entity) {
        ...
        circuits.push({ name: f.value || entity, entity, max_power: mp ? parseFloat(mp) : undefined });
      }
    });
    this.config.title = title;
    this.config.entities = entities;
    this.config.circuits = circuits;
    this.config.tariff = { threshold_w: threshold || undefined, currency: currency || undefined };
...
/* HA Lovelace Editor Integration */
class LivePowerDashboardEditor extends HTMLElement {
  setConfig(config) {
  ...
  render() {
    ...
    if (!this._hass) { this.innerHTML = '<div>Loading entities...</div>'; return; }
    ...
    this.querySelector('#editor-add-circuit')?.addEventListener('click', () => {
      (this._config.circuits ||= []).push({ name: '', entity: '', max_power: '' });
      this.render();
    });
    this.querySelectorAll('[data-remove-circuit]').forEach(btn => {
      btn.addEventListener('click', () => {
        const idx = parseInt(btn.getAttribute('data-remove-circuit'));
        if (!isNaN(idx)) { this._config.circuits?.splice(idx, 1); this.render(); }
      });
    });
  }
  ...
  _dispatch() {
    ...
    const entities = {};
    ['grid','solar','battery','load'].forEach(k => { const v = val('editor-' + k); if (v) entities[k + '_power'] = v; });
    const threshold = parseFloat(val('editor-threshold')) || undefined;
    const circuits = [];
    ...
    this._config = {
      ...
      circuits: circuits.length ? circuits : undefined,
      peak_threshold_w: threshold,
      global_max_w: globalMax > 0 ? globalMax : undefined,
      tariff: threshold ? { threshold_w: threshold, currency: val('editor-currency') || undefined } : undefined,
    };
