...
class LivePowerDashboardCard extends HTMLElement {
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `<style>${CSS}</style><ha-card class="card">
      <div class="header"><div><div class="title">${title}</div><div class="subtitle">Grid ${flow.direction} · updates with Home Assistant state changes</div></div><div class="risk ${risk.level}">${risk.level}${risk.ratio ? ` ${(risk.ratio * 100).toFixed(0)}%` : ''}</div></div>
      <div class="grid">
