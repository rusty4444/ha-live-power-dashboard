...
Remaining non-blocking validation gap: `circuits` is still permissive at [storage.py](/Users/sam.russell/projects/ha-live-power-dashboard/custom_components/live_power_dashboard/storage.py:63): non-list inputs like a string are silently iterated and dropped instead of rejected. I would tighten that before broader release, but I would not call it release-blocking for the current surface.
